...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.markLineSentinelCharactersForChangedLines_ = function(startLine, endLine) {
    ...
    var listNumber = new Array(MAX_NUM_INDENTS);
  ...
  RichTextCodeMirror.prototype.newline = function() {
    ...
    if (!this.emptySelection_()) {
      ...
      //****** The setTimeout() is a hack to apply the old values after the update has taken place.
      //****** If you do it immediately, you're not on the new line yet. I encourage you to change this. Or not. ;-)
      //****** The initial commented out lines didn't work... not sure if there's an easy way to do it right.
      //****** Maybe use updateTextAttributes? Would need the character position.
      // for (var attr in attributes)
      //    this.currentAttributes_[attr] = attributes[attr];
      var that = this;
      setTimeout(function() {
          for (var attr in attributes)
            that.setAttribute(attr, attributes[attr]);
     }, 0);
    }
  };
  ...
  RichTextCodeMirror.prototype.indent = function() {
    this.updateLineAttributesForSelection(function(attributes) {
      ...
      if (indent && indent < MAX_NUM_INDENTS) {
        attributes[ATTR.LINE_INDENT]++;
      } else if (listType) {
