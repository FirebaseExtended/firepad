...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.removeText = function(start, end, origin) {
    var cm = this.codeMirror;
    cm.scrollInfo = cm.getScrollInfo();
  ...
  RichTextCodeMirror.prototype.onCodeMirrorBeforeChange_ = function(cm, change) {
    ...
    if (change.text == ' ' && change.origin == '+input') {
      // Automatically detect a new ordered list
      var patternsOrdered = ['1.', '1-', '1)', '1/', 'I.', 'I)', 'I/', 'I-'];
      var patternsUnordered = ['-', '.'];
      var line = this.codeMirror.getLine(change.from.line);
      var cutLine1 = line.slice(line.length-1,line.length);
      var cutLineRest1 = line.slice(1, line.length-1);
      var cutLine2 = line.slice(line.length-2,line.length);
      var cutLineRest2 = line.slice(1, line.length-2);
      if ((patternsOrdered.indexOf(cutLine2) > -1) && !cutLineRest2.match('[^ ]')) {
        setTimeout(function() {
          for (var i=2;i<line.length;i++) {
            this.deleteLeft();
          }
        }.bind(this), 0);
        change.update({line: change.from.line, ch: 1},{line: change.from.line, ch: 3});
        this.toggleLineAttribute(ATTR.LIST_TYPE, 'o');
      }
      // Automatically detect a new unordered list
      else if ((patternsUnordered.indexOf(cutLine1) > -1) && !cutLineRest1.match('[^ ]')) {
        setTimeout(function() {
          for (var i=1;i<line.length;i++) {
            this.deleteLeft();
          }
        }.bind(this), 0);
        change.update({line: change.from.line, ch: 1},{line: change.from.line, ch: 2});
        this.toggleLineAttribute(ATTR.LIST_TYPE, 'u');
      }
    }
  };
  ...
  RichTextCodeMirror.prototype.onCodeMirrorChange_ = function(cm, cmChanges) {
    ...
    for (var i = 0; i < changes.length; i++) {
      ...
      var from = change.from, start = change.start, end = change.end, text = change.text, removed = change.removed, origin = change.origin;
  ...
  RichTextCodeMirror.prototype.convertCoordinateSystemForChanges_ = function(changes) {
    ...
    for (var i = changes.length - 1; i >= 0; i--) {
      ...
      newChanges.unshift({ start: start, end: start + removedText.length, removed: removedText, text: text,
        origin: change.origin, from: from});
    }
    return newChanges;
  };
  ...
  RichTextCodeMirror.prototype.markLineSentinelCharacters_ = function(line, startIndex, endIndex, listNumber) {
    ...
    if (startIndex === 0) {
      ...
      //Starts the numbering again if the indentation decreases
