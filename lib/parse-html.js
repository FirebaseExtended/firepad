...
firepad.ParseHtml = (function () {
  ...
  function parseNode(node, state, output, isChild) {
    ...
    function parseUl(node, state, output) {
    ...
    switch (node.nodeType) {
      ...
      case Node.ELEMENT_NODE:
        ...
        switch (node.nodeName.toLowerCase()) {
          ...
          case 'p':
          //remove doc headers
            if (node.getAttribute('title') == 'header') {
              return;
            }
          ...
          case 'font':
            ...
            var applyFormatting = function(face, color, size) {             
            ...
            if (state && state.textFormatting && state.textFormatting.attributes && !(function() {
                for (var i in state.textFormatting.attributes)
                  return false;
                return true;
              })()) {
              state = state.withTextFormatting(state.textFormatting);
              applyFormatting(face, color)
            } else {
              applyFormatting(face, color, size);
            }
            if (face) { state = state.withTextFormatting(state.textFormatting.font(face)); }
            if (color) { state = state.withTextFormatting(state.textFormatting.color(color)); }
            if (size) { state = state.withTextFormatting(state.textFormatting.fontSize(size)); }
  ...
  function parseChildren(node, state, output, isChild) {
    if (node.hasChildNodes()) {
      for(var i = 0; i < node.childNodes.length; i++) {
        parseNode(node.childNodes[i], state, output, isChild);
      }
    }
    return state;
  }
  ...
  function parseListItem(node, state, output) {
    ...
    parseChildren(node, state, output, true);
    ...
    for(var i = 0; i < styles.length; i++) {
      ...
      switch (prop) {
        ...
        case 'font-family':
          ...
          break;
        case 'text-indent':
          textFormatting = textFormatting.indent(val);
          break;
        case 'line-comment':
          lineFormatting = lineFormatting.cloneWithNewAttribute_(prop, val);
          lineFormatting = lineFormatting.cloneWithNewAttribute_('has-comment', true);
      }
    }
