...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    this.ref_.child('history').child(revisionId).transaction(function(current) {
      if (current === null) {
        return { a: self.userId_, o: operation.toJSON(), t: Firebase.ServerValue.TIMESTAMP };
      }
    }, function(error, committed) {
      callback(!error && committed);
    }, /*applyLocally=*/false);
  };
    ...
    while (pending[revisionId] != null) {
      ...
      if (!revision) {
          ...
          revision.author === this.userId_)
        {
          ...
          // This was someone else's change, let the editor know
