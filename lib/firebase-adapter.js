...
firepad.FirebaseAdapter = (function (global) {
  ...
  utils.makeEventEmitter(FirebaseAdapter, ['ready', 'cursor', 'operation', 'ack', 'retry', 'error']);
  ...
  FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    try {
      ...
      this.trigger('error', ex, { operation: operation.toString(), document: this.document_.toString() });
      throw ex;
    }
      ...
      self.ref_.child('history').child(revisionId).transaction(function(current) {
        ...
        if (error) {
          if (error.message === 'disconnect') {
            ...
            self.trigger('error', error, { operation: revisionData.o.toString(), document: self.document_.toString() });
            throw error;
          }
        } else {
