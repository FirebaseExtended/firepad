...
var MonacoAdapter = function () {
  ...
  function MonacoAdapter(monacoInstance) {
    ...
    this.lastDocLines = this.getModel().getLinesContent();
    ...
    this.modelChangeHandler = this.monaco.onDidChangeModel(this.onModelChange);
  ...
  MonacoAdapter.prototype.detach = function detach() {
    ...
    this.modelChangeHandler.dispose();
  ...
  MonacoAdapter.prototype.onModelChange = function onModelChange(event) {
    ...
    const oldRange = new monaco.Range(1, 1, oldLinesCount, oldLastColumLength);
    const oldValueLength = this.lastDocLines.join(newModel.getEOL()).length;

    this.onChange({
      changes: [{
        range: oldRange,
        rangeOffset: 0,
        rangeLength: oldValueLength,
  ...
  MonacoAdapter.prototype.onChange = function onChange(event) {
    ...
    if (!this.ignoreChanges) {
      var content = this.lastDocLines.join(model.getEOL());
      var offset = 0;
  ...
  MonacoAdapter.prototype.applyOperation = function applyOperation(operation) {
    ...
    opsList.forEach(function (op) {
      ...
      if (op.isRetain()) {
        ...
        const { readOnly } = _this.monaco.getConfiguration();

        if (readOnly) {
          _this.monaco.updateOptions({ readOnly: false });
        }
        ...
        if (readOnly) {
          _this.monaco.updateOptions({ readOnly });
        }
  ...
  MonacoAdapter.prototype.executeCommand = function() {
    ...
    if (multiCursorActions.includes(command)) {
      return Promise.resolve();
    }
