...
var MonacoAdapter = function () {
  ...
  function MonacoAdapter(monacoInstance) {
    ...
    this.disposables = [
      this.monaco.onDidPaste(console.log),
  ...
  MonacoAdapter.prototype.detach = function detach() {
    ...
    if (model.undo === this.callbacks.undo) {
      model.undo = this.originalUndo;
    }

    if (model.redo === this.callbacks.redo) {
      model.redo = this.originalRedo;
    }
  ...
  MonacoAdapter.prototype.onModelChange = function onModelChange(event) {
    ...
    if (this.callbacks.undo) {
      this.originalUndo = newModel.undo;
      newModel.undo = this.callbacks.undo;
    }

    if (this.callbacks.redo) {
      this.originalRedo = newModel.redo;
      newModel.redo = this.callbacks.redo;
    }
  ...
  MonacoAdapter.prototype.onChange = function onChange(event) {
    ...
    this.emit.call(this, 'change', mainOp, reverseOp);
  ...
  MonacoAdapter.prototype.operationFromMonacoChange = function (changes, contentLength) {
    ...
    for (let i = 0; i < changes.length; i++) {
      ...
      try {
        ...
        this.trigger('error', ex, mainOp.toString(), { retain });
        throw ex;
      }
    ...
    try {
      ...
      this.trigger('error', ex, mainOp.toString(), { contentLength, skippedChars });
      throw ex;
    }
  ...
  /**
   * @method emit - Event Handler
   * @param {string} event - Event name
   * @param  {...any} args - Callback arguments
   */
  MonacoAdapter.prototype.emit = function emit(event) {
  ...
  MonacoAdapter.prototype.onCursorActivity = function onCursorActivity() {
    ...
    this.cursorTimer = setTimeout(function () {
      return _this.emit('cursorActivity');
    }, 1);
  };
...
firepad.utils.makeEventEmitter(MonacoAdapter, ['error']);
