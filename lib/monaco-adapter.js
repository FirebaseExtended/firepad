 ...
 * 4. Check functioning of isDelete in applyOperation
 * @type {MonacoAdapter}
 */
...
firepad.MonacoAdapter = (function() {
  ...
  MonacoAdapter.prototype.operationFromMonacoChange = function(change) {
    ...
    console.log('docLine', this.lastDocLines);
    insert_op = new firepad.TextOperation().retain(start).insert(text).retain(0);
    delete_op = new firepad.TextOperation().retain(start)["delete"](text).retain(0);
    if (change.changes[0].text === "" && change.changes[0].rangeLength > 0) {
  ...
  MonacoAdapter.prototype.indexFromPos = function(range, upto, lines) {
    ...
    if (upto === 'start') {
      ...
      return index + range.startColumn;
    } else {
      for (row = 1; row < range.endLineNumber; row++) {
        index += lines[row-1].length + 1
      }
      return index + range.endColumn;
    }
  };
  ...
  //   this.otherCursors[clientId] = cursorRange = new monaco.Range(start.row, start.column, end.row, end.column);
  //   self = this;
