...
firepad.MonacoAdapter = (function() {
  ...
  MonacoAdapter.prototype.operationFromMonacoChange = function(change) {
    ...
    start = this.indexFromPos(change.changes, 'start');
  ...
  MonacoAdapter.prototype.indexFromPos = function(changes, upto, lines) {
    var index;
    if (lines == null) {
    ...
    index = 0;
    if (upto == 'start') {
      for (row = 0; row < changes[0].range.startLineNumber; row++) {
        start += lines[row].length + 1
      }
      return index + changes[0].range.startColumn;
    } else {
      for (row = 0; row < changes[0].range.endLineNumber; row++) {
        start += lines[row].length + 1
      }
      return index + changes[0].range.endColumn;
    }
  };
  ...
  MonacoAdapter.prototype.getCursor = function() {
    ...
    try {
      start = this.indexFromPos(this.aceSession.selection.getRange().start, 'start', this.aceDoc.$lines);
      end = this.indexFromPos(this.aceSession.selection.getRange().end, 'end', this.aceDoc.$lines);
    } catch (_error) {
      e = _error;
      try {
        start = this.indexFromPos(this.lastCursorRange.start, 'start');
        end = this.indexFromPos(this.lastCursorRange.end, 'end');
      } catch (_error) {
        e2 = _error;
