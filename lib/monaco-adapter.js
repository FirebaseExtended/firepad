...
firepad.MonacoAdapter = (function() {
  ...
  MonacoAdapter.prototype.operationFromMonacoChange = function(change) {
    ...
    delete_op = new firepad.TextOperation().retain(start)["delete"](text)
        .retain(restLength);
    ins_op_ne = new firepad.TextOperation().retain(start)["delete"](text_m)
        .insert(text).retain(restLength);
    del_op_ne = new firepad.TextOperation().retain(start)["delete"](text)
        .insert(text_m).retain(restLength);

    // dop = new firepad.TextOperation()
    //     .retain(start)["delete"](text).retain(restLength)
    //     .compose(new firepad.TextOperation().retain(start).insert(text_m)
    //     .retain(restLength));
    // iop =new firepad.TextOperation()
    //     .retain(start)["delete"](text_m).retain(restLength)
    //     .compose( new firepad.TextOperation().retain(start).insert(text)
    //     .retain(restLength));
  ...
  MonacoAdapter.prototype.applyOperationToMonaco = function(operation) {
    ...
    index = 0;
      ...
      if (op.isRetain()) {
        ...
        range = new monaco.Range(this.posFromIndex(index).row,
          this.posFromIndex(index).column,
          this.posFromIndex(index).row,
          this.posFromIndex(index).column);
        id = { major: 1, minor: 1 };
        ops = {identifier: id, range: range, text: op.text, forceMoveMarkers: true};
        ...
        id = { major: 1, minor: 2 };
        ops = {identifier: id, range: range, text: "", forceMoveMarkers: true};
        editor.executeEdits("my-source", [ops]);
      }
    }
    return this.grabDocumentState();
  ...
  MonacoAdapter.prototype.setOtherCursor = function(cursor, color, clientId) {
    ...
    if (this.otherCursors == null){
      console.log('other cursor is null');
      this.otherCursors = {};
    ...
    console.log('otherCursor', this.otherCursors);
    start = this.posFromIndex(cursor.position);
    end = this.posFromIndex(cursor.selectionEnd);
