...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    if (this.document_.targetLength !== operation.baseLength) {
      throw Error("sendOperation() called with invalid operation.");
    }
    ...
    setTimeout(function() {
      self.firebaseOn_(historyRef, 'child_added', function(revisionSnapshot) {
        try {
          ...
          self.trigger('sync_failed', error);
          utils.log('Failed sync.', error);
        }
      });
...
firepad.EditorClient = (function () {
  ...
  function EditorClient (serverAdapter, editorAdapter) {
    ...
    this.editorAdapter.registerCallbacks({
      ...
      sync_failed: function() { self.trigger('sync_failed'); }
    });
  ...
  EditorClient.prototype.sendOperation = function (operation) {
    try {
      ...
      firepad.utils.log('Failure in sendOperation()', error);
      self.trigger('sync_failed', error);
    }
  };
...
firepad.utils.makeEventEmitter(firepad.EditorClient, ['synced', 'sync_failed']);
...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    ...
    this.client_.on('sync_failed', function() {
